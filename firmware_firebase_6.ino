...
// AUTO mode trigger thresholds (with hysteresis to prevent flapping)
//   compressor reaches TRIGGER → start spraying
//   compressor drops to RELEASE → stop spraying, wait for trigger again
const float COMP_TRIGGER_C      = 38.0;
const float COMP_RELEASE_C      = 35.0;
bool autoTriggered              = false;  // true = AUTO is actively cycling

// ─────────────────────────────────────────────
//  CISTERN LEVEL — software-estimated (HRLV-MaxSonar cannot measure <30cm,
//  which is below our tank's working range, so we estimate level from pump
//  runtime instead). Tune CISTERN_DRAIN_PCT_PER_SEC to match measured flow.
// ─────────────────────────────────────────────
const float CISTERN_INITIAL_PCT       = 40.0;   // level shown on boot / after refill
const float CISTERN_DRAIN_PCT_PER_SEC = 0.05;   // %/sec while pump is on (~13min from 40→0 of pump-on time)
unsigned long lastCisternUpdateMs     = 0;
...
void readSensors() {
  ...
  // Cistern level — simulated from pump runtime (sonar bypassed).
  // Drains at CISTERN_DRAIN_PCT_PER_SEC while relay is ON.
  unsigned long nowMs = millis();
  if (lastCisternUpdateMs == 0) lastCisternUpdateMs = nowMs;
  float dtSec = (nowMs - lastCisternUpdateMs) / 1000.0f;
  lastCisternUpdateMs = nowMs;

  if (mistState == MIST_SPRAYING) {
    cisternPct -= CISTERN_DRAIN_PCT_PER_SEC * dtSec;
    if (cisternPct < 0.0f) cisternPct = 0.0f;
  }
  // Derived "distance" purely for dashboard display continuity
  cisternDistCm = CISTERN_EMPTY_CM - (cisternPct / 100.0f) * (CISTERN_EMPTY_CM - CISTERN_FULL_CM);
...
void updateMistingStateMachine() {
  ...
  // AUTO mode — gated by compressor temperature with hysteresis.
  // Only run the spray/pause cycle when compressor is HOT enough.
  if (!isnan(compressorTempC)) {
    if (!autoTriggered && compressorTempC >= COMP_TRIGGER_C) {
      autoTriggered = true;   // crossed trigger → start cooling
    } else if (autoTriggered && compressorTempC <= COMP_RELEASE_C) {
      autoTriggered = false;  // dropped to release → stop cooling
    }
  }

  // If not triggered, sit idle (relay off) and wait for compressor to warm up.
  if (!autoTriggered) {
    setRelay(false);
    enterState(MIST_IDLE);
    return;
  ...
  // AUTO + triggered — run spray/pause cycle
  switch (mistState) {
    case MIST_IDLE:
    case MIST_BLOCKED:
...
void pushTelemetry() {
  ...
  // AUTO mode threshold gate (shown on dashboard as Armed/Triggered)
  json.set("comp_trigger_c",     COMP_TRIGGER_C);
  json.set("comp_release_c",     COMP_RELEASE_C);
  json.set("auto_triggered",     autoTriggered);
...
void handleCommandStream() {
  ...
  if (cmd == "ON") {
    ...
    pumpMode = MODE_AUTO;
    autoTriggered = false;   // re-evaluate trigger from current temp
    enterState(MIST_IDLE);  // restart the cycle cleanly
    resetEffectivenessSession();
    Serial.println("Misting: AUTO (waits for compressor ≥ 38°C, releases at ≤ 35°C)");
  } else if (cmd == "REFILL") {
    cisternPct = CISTERN_INITIAL_PCT;
    Serial.printf("Cistern refilled to %.0f%%\n", CISTERN_INITIAL_PCT);
  }
}
